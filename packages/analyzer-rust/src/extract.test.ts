...
describe("snapshotRustWorkingTree", () => {
  it("extracts struct/trait/enum/impl-method symbols + pub visibility + use deps", async () => {
    ...
    expect(
      syms.some((s) => s.parentClass === "Greeter" && s.name === "new" && s.kind === "method"),
    ).toBe(true);
...
describe("qualified-path references (tree-sitter only)", () => {
  ...
  afterAll(async () => {
    await rm(qp, { recursive: true, force: true });
  });
  ...
  it("resolves a fully-qualified call with no `use` into an import edge + symbol ref", async () => {
    ...
    const edge = imports.find((e) => e.from === "file:src/app.rs" && e.to === "file:src/math.rs");
    expect(edge?.resolved).toBe(true);
    expect(
      edge?.type === "imports"
        ? edge.symbolImports?.some((s) => s.fromSymbolName === "run" && s.toSymbolName === "add")
        : false,
    ).toBe(true);
  });
  ...
  it("links a Type::method() associated call to the method symbol", async () => {
    ...
    expect(
      edge?.type === "imports"
        ? edge.symbolImports?.some((s) => s.fromSymbolName === "make" && s.toSymbolName === "new")
        : false,
    ).toBe(true);
  });
  ...
  it("does not invent edges for std associated calls (String::new)", async () => {
    ...
    expect(imports.some((e) => e.resolved && e.to === "file:src/String.rs")).toBe(false);
  });
});
...
describe("cargo workspace cross-crate resolution", () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(join(ws, "Cargo.toml"), `[workspace]\nmembers = ["crates/*"]\n`);
    await mkdir(join(ws, "crates/mylib/src"), { recursive: true });
    await writeFile(
      join(ws, "crates/mylib/Cargo.toml"),
      `[package]\nname = "mylib"\nversion = "0.1.0"\n`,
    );
    await writeFile(join(ws, "crates/mylib/src/lib.rs"), `pub struct Widget { pub id: u32 }\n`);
  ...
  it("resolves a sibling-crate use to that crate's source, keeps externals external", async () => {
    ...
    expect(imports.some((e) => e.specifier?.startsWith("serde") && e.external)).toBe(true);
    // `Widget` usage in run() becomes a symbol reference to mylib's Widget
    const crossEdge = imports.find(
      (e) => e.from === "file:crates/app/src/lib.rs" && e.to === "file:crates/mylib/src/lib.rs",
    );
    expect(
