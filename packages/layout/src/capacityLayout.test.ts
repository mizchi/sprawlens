...
describe("capacityStep convergence under many tied weights (regression)", () => {
  ...
  // of equal-target cells kept trading the "empty" role, each recovery jump
  // shoving a neighbor empty, so maxRelativeError swung (these seeds: tailMax
  // 1.3-2.2) instead of settling. Geometry churn from the Lloyd move sustained
  // it — the oscillation kept the error high, which kept Lloyd at full strength,
  // which kept the churn going. Annealing the Lloyd rate once a solve has run a
  // while (see lloydDamping) breaks that self-sustaining loop and the cycle
  // decays to convergence. These (clip, n, seed) triples oscillated after the
